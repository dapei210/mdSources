...
redis调用fork函数产生一个子进程，快照持久化完全交给子进程来处理，父进程继续处理客户端请求。使用COW(Copy On Write)进行数据段页面的分离，数据段是由很多操作系统的页面组合而成，当父进程对其中一个页面的数据进行修改时，会将被共享的页面复制一份分离出来，然后对这个复制的页面进行修改，这时子进程相应的页面时没有变化对的，这是进程产生时那一瞬间的数据。
...
#### 混合持久化
快照恢复会丢失大量数据，AOF日志重放太慢；混合持久化，将rdb文件的内容和增量的AOF日志文件存在一起，增量的AOF日志文件时自持久化开始到持久化结束的这段时间发生的，redis重启的时候，可以先加载rdb内容，然后再重放增量AOF日志，就可以完全替代之前的AOF全量文件重放，重启效率因此得到大幅提升。
...
### redis事务
每个事务的操作涉及到三个步骤：开始、提交以及回滚，其对应redis事务为multi, exec, discard

所有的指令在exec之前不执行，而是缓存在服务器的一个事务队列中，服务器一旦收到exec指令，才开始执行整个事务队列，执行完毕后一次性返回所有指令的运行结果。因为redis单线程特性，不用担心自己在执行队列的时候被其他指令打扰，可以保证“原子性”执行， 这个原子性仅仅是满足了事务的“隔离性”中的串行化---当前执行的事务有着不被其他事务打断的权利
...
#### watch
watch机制，是一种乐观锁，可以作为解决并发修改的方法。

watch会在事务开始之前盯住一个或多个关键变量，当事务执行时，也就是服务收到了exec指令要顺序执行缓存的事务队列时，redis会检查关键变量自watch之后是否被修改（包括当前事务所在的客户端）。如果关键变量被人动过了，exec指令就会返回NULL回复告诉客户端事务执行失败，这个时候客户端一般会选择重试
